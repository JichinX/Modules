...
public class MediaRecorderActivity extends Activity implements
    ...
    public static void goSmallVideoRecorder(Activity context, String overGOActivityName, MediaRecorderConfig mediaRecorderConfig) {
        if (mediaRecorderConfig.isGO_HOME()) {
            context.startActivityForResult(new Intent(context, MediaRecorderActivity.class).putExtra(MEDIA_RECORDER_CONFIG_KEY, mediaRecorderConfig), mediaRecorderConfig.getRESULT_FLAG());
        } else {
    ...
    private void initData() {
        ...
        GO_BACK = mediaRecorderConfig.isGO_HOME();
...
//                    mRecordDelete.setVisibility(View.GONE);
    ...
    public void onEncodeComplete() {
        ...
        intent.putExtra(MediaRecorderActivity.VIDEO_FILE_NAME, mMediaObject.getBaseName());
        if (GO_BACK) {
    ...
    public static class RecordHandler extends Handler {
        private MediaRecorderActivity recorderActivity;

        public RecordHandler(MediaRecorderActivity mediaRecorderActivity) {
            recorderActivity = mediaRecorderActivity;
        }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case HANDLE_INVALIDATE_PROGRESS:
                    if (recorderActivity.mMediaRecorder != null && !recorderActivity.isFinishing()) {
                        if (recorderActivity.mMediaObject != null && recorderActivity.mMediaObject.getMedaParts() != null && recorderActivity.mMediaObject.getDuration() >= recorderActivity.RECORD_TIME_MAX) {
                            recorderActivity.mTitleNext.performClick();
                            return;
                        }
                        if (recorderActivity.mProgressView != null) {
                            recorderActivity.mProgressView.invalidate();
                        }
                        // if (mPressedStatus)
                        // titleText.setText(String.format("%.1f",
                        // mMediaRecorder.getDuration() / 1000F));
                        if (recorderActivity.mPressedStatus) {
